// =============================================================================
// VINVITE — THEME 09: KAMAR VINTAGE — render logic
//
// Structure note (lesson learned from theme 08): all UI/animation wiring
// runs FIRST, before any data-dependent content rendering, and each
// content block is wrapped in try/catch so one bad field can't silently
// stop every animation that comes after it.
// =============================================================================
...
window.renderInvitation = function (data) {
  ...
  document.querySelectorAll(".brass-rule").forEach(el => ruleIO.observe(el));

  const sunburstIO = new IntersectionObserver((entries) => {
    entries.forEach(en => { if (en.isIntersecting) { en.target.classList.add("is-visible"); sunburstIO.disconnect(); } });
  }, { threshold: 0.4 });
  document.querySelectorAll(".sunburst-wrap").forEach(el => sunburstIO.observe(el));
  ...
  try {
    if (data.cover_image_url) {
      ...
      if (cover) {
        cover.style.backgroundImage = `linear-gradient(rgba(59,52,43,.6), rgba(59,52,43,.78)), url('${data.cover_image_url}')`;
        cover.style.backgroundSize = "cover";
        cover.style.backgroundPosition = "center";
        cover.style.filter = "sepia(.3)";
      }
    }
...
// -----------------------------------------------------------------------------
// FILM-PROJECTOR FLICKER — replaces the engine's plain fade for this theme.
// On click: a rapid multi-step opacity/brightness flicker plays (CSS
// keyframes, see .is-flickering in style.css), then the cover is removed.
// -----------------------------------------------------------------------------
function initFilmFlicker() {
  const cover = document.getElementById("cover-screen");
  ...
  btn.addEventListener("click", () => {
    cover.classList.add("is-flickering");
