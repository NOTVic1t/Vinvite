// =============================================================================
// VINVITE — THEME 11: CAHAYA BALI — render logic
// =============================================================================
...
window.renderInvitation = function (data) {
  ...
  document.querySelectorAll(".lotus-divider").forEach(el => io.observe(el));
  ...
  try {
    // Cover background photo is admin-controlled and independently
    // togglable — when set, a teal scrim keeps the gate/text legible.
    if (data.cover_image_url) {
      const cover = document.getElementById("cover-screen");
      if (cover) {
        cover.style.backgroundImage = `linear-gradient(rgba(20,63,60,.62), rgba(20,63,60,.8)), url('${data.cover_image_url}')`;
...
// page for as long as it's open (independent of scroll position).
// -----------------------------------------------------------------------------
function spawnPetals() {
  ...
  function spawnOne() {
    ...
    p.innerHTML = '<svg viewBox="0 0 40 40"><use href="#frangipani" /></svg>';
    field.appendChild(p);
    setTimeout(() => p.remove(), 19000);
  }

  for (let i = 0; i < 5; i++) setTimeout(spawnOne, i * 1100);
  setInterval(spawnOne, 1800);
}
