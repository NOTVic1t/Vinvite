// =============================================================================
// VINVITE — THEME 06: ZAITUN ISLAMI — render logic
// =============================================================================
...
window.renderInvitation = function (data) {
  ...
  // Cover background photo (optional) — deep-green overlay keeps the mihrab
  // outline & gold text legible on top.
  if (data.cover_image_url) {
    const cover = document.getElementById("cover-screen");
    if (cover) {
      cover.style.backgroundImage = `linear-gradient(rgba(44,58,34,.88), rgba(44,58,34,.94)), url('${data.cover_image_url}')`;
  ...
  // Quote: Arabic ayat stays fixed (verified Qur'anic text, not admin-edited);
  // only the Indonesian translation/source are swappable via admin, same as
  // every other theme's quote section.
  ...
  // Unwan divider + islimi border strip: both draw themselves in once their
  // section scrolls into view (same IntersectionObserver hook, [data-rule]).
...
// -----------------------------------------------------------------------------
// COVER SHIMMER — plays the gold shimmer sweep once, right after the cover
// is opened (rather than immediately on load, so the visitor actually sees it).
// -----------------------------------------------------------------------------
function initCoverShimmer() {
  const btn = document.getElementById("open-invitation-btn");
  const names = document.querySelector(".cover-names");
  if (!btn || !names) return;
  btn.addEventListener("click", () => {
    setTimeout(() => names.classList.add("is-shimmering"), 250);
  }, { once: true });
}
